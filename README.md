...
```bash
python3 -m donjon                # interface terminal (curses)
python3 -m donjon --seed 42      # partie reproductible
python3 -m donjon --depth 10     # 10 étages avant la victoire
```

Touches : `hjkl` + `yubn` (ou les flèches) pour se déplacer et attaquer,
`,` ramasser, `>` descendre, `.` attendre, `i` inventaire, `?` aide, `q` quitter.
Dans l'inventaire : la lettre de l'objet, puis `u` utiliser, `e` équiper,
`t` lancer, `d` poser.
...
2. **UI découplée du moteur.** `game.py` n'importe rien de `ui.py` : on peut
   brancher une autre interface, ou aucune (tests, bot).
